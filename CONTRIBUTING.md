...
If you want to add, correct, or improve the content of VivaSLO, you will be most interested in the following files:
...
Where Markdown is insufficient for our needs, we sometimes substitute HTML.
...
1. **Our audience is people in SLO County who are either experiencing homelessness or are threatened with it.**
   ...
   If information is not something that our audience can use, it does not belong in this guide.
1. **Our audience has a variety of challenges.**
   ...
   They are unlikely to have their own FAX machines, personal computers, scanners, or other such large devices (but may be able to access these through e.g. libraries and other agencies).
   ...
   Use simple verbs (e.g. “cause”) rather than phrasal verbs (e.g. “bring about”) as those are easier for machine translators and for people who speak English as a second language.
...
1. **Make all changes to both the English and Spanish versions.**
   ...
   You can use a machine translator like [Google Translate](https://translate.google.com/) or a good AI chat bot or coding assistant (e.g. Claude Code) if you do not know Spanish yourself.
   This is another good reason to use simple sentence structure, with simple verbs and tenses, and avoiding idioms and jargon: these things make machine translations more accurate.
   Sometimes information needs to be different between the English and Spanish versions: for instance when a resource has two phone numbers (one for English speakers and one for Spanish speakers).
   Typically, the names of programs and agencies remain in English in the Spanish-language pages.
   However some programs and agencies have official Spanish translations of their names; if you can discover these, use these translations instead.
...
1. **Add source annotations.**
   ...
   Often, a URL makes for a good *Source*, but here are some other options:
   ...
   The best sources are things like main pages on the websites run by the agencies or programs, conversations with responsible people at those agencies or programs, or recent social media posts by the agencies or programs.
   ...
   Least reliable are aggregator sites like Yelp or Google that often have information about programs or agencies, but that are not very good about ensuring that the information is accurate or up-to-date.
1. **Use consistent markup and typography.**
   - Always link web page URLs, like this: `[website.com/page](https://www.website.com/page/)`.
     Use the `https` form of the URL unless it is only available at the `http` form.
   ...
   - Always give phone numbers in 123-456-7890 format, and always hyperlink them using the `tel:` protocol, like this:
     ...
     An exception to this format is smaller numbers like 211 (`[211](tel:+1-211)`).
   ...
   - For phone numbers that are meant to receive texts rather than calls, use the `sms:` protocol, like this: `[123-456-7890](sms:+1-123-456-7890)`.
     If the reader is meant to start a text conversation with a particular starting message, you can add that in this way:
...
This project is developed in a public GitHub repo.

It is deployed on Cloudflare.

Changes to the `main` branch of this repo *automatically* deploy to all users, so be cautious about merging to `main`.
Make your changes in a development branch and test them there before merging.
